...
from methods.common.configure_model import get_h2o_args, get_topk_args, get_pca_args, get_save_tensor_args, get_energy_args, get_eviction_args
...
if __name__ == "__main__":
    ...
    parser.add_argument("--crosscov-mode", type=str, default="select", choices=["select", "compress", "evict", "keydiff", "keynorm"], help="CrossCov runtime mode: select / compress / evict (ours), keydiff & keynorm (eviction baselines)")
    parser.add_argument("--evict-ratio", type=float, default=0.5, help="fraction of tokens to evict (eviction-family modes)")
    parser.add_argument("--keep-tokens", type=int, default=0, help="absolute KV budget to keep; overrides --evict-ratio when >0 (matches KeyDiff budget protocol)")
    parser.add_argument("--sink-tokens", type=int, default=16, help="initial tokens protected from eviction")
    parser.add_argument("--recent-window", type=int, default=64, help="most-recent tokens protected from eviction")
    ...
    parser = get_energy_args(parser)
    parser = get_eviction_args(parser)
